								...
								<div class="card">
									<div class="card-body">
										<div class="d-flex no-block align-items-center">
											<div>
												<h6 class="">Total TRX USD</h6>
												<h3 class="m-0">{{number_format($tusd,2)}}USD</h3>
											</div>
											<div class="ml-auto">
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6  col-sm-6">
								<div class="card">
									<div class="card-body">
										<div class="d-flex no-block align-items-center">
											<div>
												<h6 class="">Total TRX {{$currency->symbol}}</h6>
												<h3 class="m-0">{{number_format($tunit,8)}}{{$currency->symbol}}</h3>
											</div>
											<div class="ml-auto">
												<img class="w-7 h-7" src="{{url('/')}}/back/images/crypto-currencies/round-outline/{{$currency->image}}" alt="image">
											</div>
								...
								<div class="card">
									<div class="card-header">
										<div class="card-title">All {{$currency->name}} TRX History</div>
											...
											<table id="example" class="table border table-bordered mb-0 text-nowrap">
												...
												@foreach($all as $data)
													<tr>
														...
														<td class=" @if($data->type == 'send') text-danger @else  text-success @endif">{{$data->amount}} {{$currency->symbol}}<br>
														<small>{{number_format($data->usd,2)}} USD</small></td>
													@if($data->type == 'send')
														<td><span class="badge badge-danger-light fs-12">Send</span></td>
													@else
														<td><span class="badge badge-success-light fs-12">Received</span></td>
													@endif
													</tr>
												@endforeach
